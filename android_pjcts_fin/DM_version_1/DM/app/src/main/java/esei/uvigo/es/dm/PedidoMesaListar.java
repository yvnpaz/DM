...
public class PedidoMesaListar extends Activity{
	TextView Mesa;
	TextView Lista;
	SQLiteDatabase db;
	...
	public void onCreate(Bundle savedInstanceState){
		...
		setContentView(es.uvigo.esei.dm1516.p45.R.layout.pedido_mesa_listar);
        Intent myCallerIntentHandler = getIntent();
		Bundle myBundle = myCallerIntentHandler.getExtras();
		num_mesa = myBundle.getInt("mesa");
		TextView Mesa = (TextView) findViewById(es.uvigo.esei.dm1516.p45.R.id.Mesa);
		TextView Lista = (TextView) findViewById(es.uvigo.esei.dm1516.p45.R.id.Lista);
		Mesa.setText("Pedido Mesa "+num_mesa);
		...
		try{
			...
			toshow = query();
			if(toshow.isEmpty()){
				Lista.setText("No hay ningun pedido");
			}else{
			...
			for(int j=0;j<Producto.size();j++){
				mostrar+="Producto:"+Producto.get(j)+"\n"+"Cantidad:"+Cantidad.get(j)+"\n";
			}
